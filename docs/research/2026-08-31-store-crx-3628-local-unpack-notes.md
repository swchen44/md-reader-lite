...
- 主世界（page.evaluate）`fetch(location.href)` → `TypeError: Failed to fetch`（預期：一般頁面無 file:// fetch 特權）。
- **content-script 隔離世界 fetch(file://) 亦失敗**（2026-09-01 補測，`scratchpad/pw/spike-cs-fetch.mjs`：注入探針到 main.ts、真實 content script 執行 `fetch(location.href)`→`TypeError: Failed to fetch`；同探針在 http 頁 `ok:200` 正確解碼）。**結論：只有 SW 能 fetch file://，content script 不能**——故 SW/bgFetch 架構是必要的（非可繞過），設計審查「SW 可能不必要」的疑點排除。連帶：Lite 現有 content-script `fetch(location.href)` polling 在 file:// 恆失敗 →catch→ 無重渲染，**polling 在 file:// 是 inert，與 charsetCompat bgFetch 無競態**。
- 結論方案：content script 偵測 `file://` + `charsetCompat` on → 送訊息給 SW → SW `fetch` + `arrayBuffer` + `TextDecoder('utf-8')` → 回傳強制 UTF-8 文字 → content script 重渲染。**SW 安全用精確 URL 比對**（`sender.url === data.url` + file: 限縮 + `sender.id === chrome.runtime.id`），**非同源檢查**——因實測 Chromium 的 `file://` origin 恆為常數 `"file://"`、對 file⇄file 無區辨力（詳見 spec `2026-09-01-charset-compat-design.md` 安全節）。charset 欄位維持死欄位（功能=強制 UTF-8）。

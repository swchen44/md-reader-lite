...
中文摘要：本擴充零資料蒐集、零遙測、零外部請求；所有渲染在本機完成，設定僅存於裝置上的 chrome.storage.local。不申請任何主機權限——僅有的網路請求是頁面情境本身發出的同源請求：重新讀取你開啟的 Markdown 檔（自動刷新）與（啟用目錄樹時）其所在資料夾的列表。本機文件可選擇性授權唯讀資料夾存取（瀏覽器原生選擇器），授權僅存於本機瀏覽器，清除網站資料即可撤銷。於 GitHub raw 頁面使用檔案樹時會匿名呼叫 GitHub 公開 API 列目錄，無 token、無其他資料傳輸。
